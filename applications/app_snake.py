...
class frame_Snake(ctk.CTkFrame):
    ...
    # --- Configuration ---
        ...
        # Canvas
        cnv = tk.Canvas(self, width=TAILLE_CASE*LARGEUR, height=TAILLE_CASE*HAUTEUR, bg="black", highlightthickness=0)
        cnv.pack(pady=20)

        # Bouton restart
        restart_btn = ctk.CTkButton(self, text="Restart", command=lambda: init_game())

        def init_game():
            # Annuler l'ancien mouvement si on restart en plein milieu
            if game["after_id"]:
                self.after_cancel(game["after_id"])
        ...
        def move():
            ...
            if x<0 or x>=LARGEUR or y<0 or y>=HAUTEUR or (x,y) in game["snake_body"]:
                ...
                cnv.create_text(LARGEUR*TAILLE_CASE//2, HAUTEUR*TAILLE_CASE//2, text="GAME OVER\nAppuyez sur ESPACE\npour démarrer", fill="white", font=("Arial", 30))
            ...
            draw()
            game["after_id"] = self.after(100, move)
        ...
        def show_start_screen():
            cnv.delete("all")
            cnv.create_text(LARGEUR * TAILLE_CASE // 2,HAUTEUR * TAILLE_CASE // 2,text="Appuyez sur ESPACE\npour démarrer",fill="white",font=("Arial", 20),justify="center")
